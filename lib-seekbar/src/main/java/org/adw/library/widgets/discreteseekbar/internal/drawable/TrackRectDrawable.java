...
public class TrackRectDrawable extends StateDrawable {
    ...
    void doDraw(Canvas canvas, Paint paint) {
        ...
        canvas.drawCircle(getBounds().left, getBounds().top + getBounds().height() / 2, getBounds().height() / 2, paint);

        canvas.drawCircle(getBounds().right, getBounds().top+getBounds().height() / 2, getBounds().height() / 2, paint);
    }
