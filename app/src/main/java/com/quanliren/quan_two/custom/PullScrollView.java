...
//                Math.abs(mInitTop - mCurrentTop), 0);
        ...
        int toTop = mContentRect.top;

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            top -= com.quanliren.quan_two.util.Util.getStatusBarHeight(getContext());
            toTop -= com.quanliren.quan_two.util.Util.getStatusBarHeight(getContext());
        }

        // 开启移动动画
        TranslateAnimation innerAnim = new TranslateAnimation(0, 0, top, toTop);
